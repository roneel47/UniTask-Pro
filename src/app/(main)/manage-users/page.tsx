...
import { ChevronUp, Edit3, Trash2, UserX, ShieldAlert, RotateCcw } from "lucide-react";
...
export default function ManageUsersPage() {
  ...
  const { users, updateUser, deleteUser, getUsersByFilter, isLoading: dataIsLoading } = useData();
  ...
  const handleSaveChanges = async (updatedFields: Partial<User>) => {
    ...
    // Prevent changing own role or Master Admin's role
            ...
            {filteredUsers.length > 0 ? filteredUsers.map(user => (
              <TableRow key={user.id}>
                ...
                <TableCell className="text-right space-x-1">
                  ...
                  {user.role === 'student' && parseInt(user.semester, 10) < 8 && (
                     <AlertDialog>
